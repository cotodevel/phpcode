...
function db_show_modif($dataarg){
global $separador;global $sep_int;$data=decrypt_db($dataarg);$check_fid=0;$check_table=0;$campo=0;
...
$cnt_reg=0;
!isset($rs)?$nrsfid=0:$nrsfid=count($rs);
...
foreach($db as $dbid=>$dbname){
	//imprime DB
	echo "<td><center><a href='./modif_bd.php?det=".$dbid."'><font color='green' size='+2'>Base de Datos:[<b>".$dbname."</b>]</font><p></p></a>";				
				...
				for($j=0;$j<$nreg;$j++){
					...
					echo "<font color='red' size='+1'>Tabla:[<b>".$tab_j[1]."</b>]</font><p></p>";
						//mientras estamos aca, abrimos otro ciclo, para asociar los field ids
							...
							echo "<center> <b><font color='blue' size='4'>Nº Campos:".$campo."</font></b></center><p></p>";
...
//gui
echo "<center><font size='+1'><b>*Haga click en la Base de Datos que desea modificar*</b><p></p><a href='javascript:window.close()'>Cerrar Ventana</a></font><p></p>";
//muestro db show, pero interactivo. Hago click en el campo que quiero ingresar data
db_show_modif($_SESSION['data_main']);
echo "</center>";
?>