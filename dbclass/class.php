...
function db_show($dataarg){
	...
	$nreg=count($temp);
	for($i=0;$i<$nreg;$i++){
		$db[$temp[$i][0]]=$temp[$i][1];
	}
	for($i=0;$i<$nreg;$i++){
		$tk[]=$temp[$i][2].$sep_int.$temp[$i][3].$sep_int.$temp[$i][0];
	}
	
	for($i=0;$i<$nreg;$i++){
		$fd[]=$temp[$i][4].$sep_int.$temp[$i][2];
	}
	$data=null;
	$tdata=arr_unique($tk);
	$tk_uniq=arr_unique($tk);
	$cnt_tk=count($tk_uniq);
	$cnt_fd=count($fd);
...
function exp_data($data){
	error_reporting(0); //don't screw up HTTP Header on output
	global $separador;
	$val=NULL;$data=decrypt_db($data);
	switch(strlen($data)>10):
		case(true):
			$enc=(encrypt($data));
			$dump=$enc['encoded'].$separador.$enc['w_del'].$separador.$enc['password'];
			unset($data,$enc);
...
function build_db($data,$par){
	...
	if($par==1){
		...
		if ((!$cid) || (!$resultado)){
			echo "<font color='red'><b>ERROR, NO HAY INTERNET. No se puede crear la BD sin internet.</b></font>";
		}
			
		else{
			if(ftp_pasv ($cid, true)) 
				echo "ftp modo pasivo ok!";
			//3.create db
			if(ftp_fput($cid,$dir_remota.$filedr.$db_nm,$fh, FTP_BINARY )==true) { 
				fclose($fh); 
				return true;
			}
			else {
				echo "<b>BD no creada en servidor. No hay acceso a internet o problemas con FTP.</b>"; exit(); 
			}
		}
		return true;
	}
...
function destroy_data($data){
	...
	//si el archivo existe en ruta destino, lo eliminamos y creamos uno nuevo
		...
		if(ftp_close($cid)==true) 
			echo "<p></p><font color='green'><b>Transacciones FTP finalizadas correctamente!!</b></font><p></p>";
	}
...
function sh_data($data,$arg){
	global $separador,$sep_int,$fdefdata; 
	$db=null;
	...
	switch($dec["pw_status"]=="OK"):
		case(true):
					...
					for($i=0;$i<$cnt_dbfid;$i++){
						...
						switch($dbfid[$i]):
							...
							case($arg[0]):
								...
								endswitch;
								if($fdata==false) 
								echo "<b>No hay registros ingresados en el sistema!</b><p></p>";
			...
			switch(!isset($dbfid)):
				case(true):
					echo "<center><b>No se encontraron estructuras de la base de datos. Posiblemente fueron eliminadas en este instante.<p></p> La estructura se obtiene desde la opcion (Reconstruir y Exportar una BD con el formato correcto)</b>.";echo "<p></p>";echo "<a href='javascript:window.close()'>Cerrar Ventana</a></center>";
				break;
			endswitch;
				
			switch(!isset($rsdata)):
				case(true):
					echo "<center><b>No se encontraron registros en el sistema. Debes ingresar uno al menos.</b>.";echo "<p></p>";echo "<a href='javascript:window.close()'>Cerrar Ventana</a></center>";
				break;
			endswitch;
		break;
	endswitch;
...
function dbsh_modif_table($dataarg){
	global $separador,$sep_int;
	$data=decrypt_db($dataarg);
	$check_fid=0;$check_table=0;
	...
	//si no hay datos en memoria, error
	if (!isset($temp)) { 
			echo 'data invalida en memoria, re-ingrese la BD'; exit(); 
	}
	//gui
	echo "<center>";$nreg=count($temp);
	for($i=0;$i<$nreg;$i++){
		$db[$temp[$i][0]]=$temp[$i][1];
	}
	for($i=0;$i<$nreg;$i++){
